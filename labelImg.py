...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def loadFile(self, filePath=None):
        ...
        scrolls = [self.scrollBars[Qt.Horizontal].value(),self.scrollBars[Qt.Vertical].value()]
        self.resetState()
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            print(self.scrollBars[Qt.Horizontal].value())
            ...
            self.scrollBars[Qt.Horizontal].setValue(scrolls[0])
            self.scrollBars[Qt.Vertical].setValue(scrolls[1])
            return True
        return False
    ...
    def openNextImg(self, _value=False):
        ...
        if self.filePath is None:
            ...
            if currIndex + 1 < len(self.mImgList):
                filename = self.mImgList[currIndex + 1]
                self.interpNextLabel(currIndex)
        if filename:
            self.loadFile(filename)
    def interpNextLabel(self, currIndex):
        if self.defaultSaveDir is not None and  1 <= currIndex < len(self.mImgList)-1:
            last_file = os.path.basename(self.mImgList[currIndex-1])
            last_file = os.path.splitext(last_file)[0]
            current_file = os.path.basename(self.mImgList[currIndex])
            current_file = os.path.splitext(current_file)[0]
            next_file = os.path.basename(self.mImgList[currIndex+1])
            next_file = os.path.splitext(next_file)[0]
            last_label_file = os.path.join(self.defaultSaveDir, last_file + XML_EXT)
            current_label_file = os.path.join(self.defaultSaveDir, current_file + XML_EXT)
            next_label_file = os.path.join(self.defaultSaveDir, next_file + XML_EXT)
            if os.path.exists(last_label_file) and os.path.exists(current_label_file) and not os.path.exists(next_label_file):
                # self.loadLabels()
                lastVocParseReader = PascalVocReader(last_label_file)
                last_shape = lastVocParseReader.getShapes()[0][1]
                currentVocParseReader = PascalVocReader(current_label_file)
                current_shape = currentVocParseReader.getShapes()[0][1]

                next_points = np.array(current_shape) + np.array(current_shape) - np.array(last_shape)
                next_shape = {}
                next_shape['points'] = next_points
                next_shape['label'] = 'object'
                next_shape['difficult'] = False
                next_shapes = []
                next_shapes.append(next_shape)
                next_labelFile = LabelFile()
                next_labelFile.savePascalVocFormat(next_label_file, next_shapes, self.mImgList[currIndex+1], self.imageData,
                                                   self.lineColor.getRgb(), self.fillColor.getRgb())
